...
/** Thrown when mail is required but no provider is configured. */
...
function sender(): string {
  return process.env.MAIL_FROM || 'Awesome MCP <onboarding@resend.dev>';
}
...
export async function sendMail(message: MailMessage): Promise<void> {
  ...
  if (!key) {
    if (!isDevFallbackAllowed()) throw new MailNotConfiguredError();
