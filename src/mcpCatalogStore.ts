...
/** The local URL normalizer seedDefaultCatalogs hands to its per-provider helpers. */
type NormalizeUrl = (url: string | undefined, defaultPath: string) => string;

/**
 * Seed the Redmine catalog row.
 *
 * Extracted from seedDefaultCatalogs because its env gating is the most
 * branch-heavy of any provider here, and inlining it pushed that function past
 * the cognitive-complexity budget.
 */
async function seedRedmineCatalog(normalizeUrl: NormalizeUrl): Promise<void> {
  // Redmine MCP
  //
  ...
  const redmineOauthBaseUrl = stripTrailingSlashes(process.env.REDMINE_BASE_URL || '');
