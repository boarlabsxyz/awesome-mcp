...
- **HubSpot engagement writes (`createNote`/`createTask`/`logCall`/`logMeeting`):** create the engagement standalone then attach it via a v4 **default** association (`PUT /crm/v4/objects/{type}/{id}/associations/default/{toType}/{toId}`), avoiding the direction-sensitive association-type-ID table. `hs_timestamp` defaults to now when omitted (the #1 documented create failure). `deleteEngagement(engagementType, engagementId)` is the cleanup counterpart — HubSpot's DELETE archives the engagement (off every timeline, recoverable from the recycling bin), which is what makes the create tools safely testable against a real portal; it needs the same activity scope as the writes. Success only claims a timeline attachment once the association call returns 2xx; a failed association is reported as an orphaned engagement, not a silent success. **Scope caveat:** company/contact scopes do NOT cover activity writes, and an *unknown* OAuth scope breaks the whole consent flow — so the activity-write scope is intentionally NOT hardcoded in `oauthScopes`. Enable it on the registered HubSpot app, then add its exact string (candidate `crm.objects.notes.write`) to `mcpCatalogStore.ts` and reconnect. Until then these tools 401/403 (surfaced verbatim).
