...
- **HubSpot `searchData` not registered:** the reference's 16th tool did semantic search over a local FAISS vector index; this codebase has no vector store, so the tool is intentionally omitted rather than advertised as a throwing stub. The 26 registered HubSpot tools are implemented against the official HubSpot REST API. (`searchCompanies`/`searchContacts` cover the CRM search endpoint — the free-text/property lookup the omitted FAISS tool was reaching for.)
